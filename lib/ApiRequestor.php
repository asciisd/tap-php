...
class ApiRequestor
{
    ...
    private static function _telemetryJson($requestTelemetry)
    {
        ...
        if ($result != false) {
            ...
            Tap::getLogger()->error("Serializing telemetry payload failed!");
            return "{}";
        }
    }
    ...
    {
        $msg = isset($errorData['description']) ? $errorData['description'] : null;
    ...
    private static function _specificOAuthError($rbody, $rcode, $rheaders, $resp, $errorCode)
    {
        ...
        $message = isset($resp['message']) ? $resp['message'] : $errorCode;
    ...
    {
        $uaString = 'Tap/v2 PhpBindings/' . Tap::VERSION;

        $langVersion = phpversion();
        $uname = php_uname();

        $appInfo = Tap::getAppInfo();
        $ua = [
        ...
        $defaultHeaders = [
            'X-Tap-Client-User-Agent' => json_encode($ua),
            'User-Agent' => $uaString,
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        // X-Tap-Client-User-Agent header via the optional getUserAgentInfo()
        // method
        $clientUAInfo = null;
        ...
        if (Tap::$apiVersion) {
            $defaultHeaders['Tap-Version'] = Tap::$apiVersion;
        }
        ...
        if (Tap::$enableTelemetry && self::$requestTelemetry != null) {
            $defaultHeaders["X-Tap-Client-Telemetry"] = self::_telemetryJson(self::$requestTelemetry);
        }
