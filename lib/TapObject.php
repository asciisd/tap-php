 ...
 * @package Tap
 */
class TapObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    protected $_opts;
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToTapObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_values) && array_key_exists($k, $this->_values)) {
            ...
            $message = "Tap Notice: Undefined property of $class instance: $k. "
                . "HINT: The $k attribute was set in the past, however. "
                . "It was then wiped when refreshing the object "
                . "with the result returned by Tap's API, "
                . "probably as a result of a save(). The attributes currently "
                . "available on this object are: $attrs";
            Tap::getLogger()->error($message);
            return $nullval;
        } else {
            $class = get_class($this);
            Tap::getLogger()->error("Tap Notice: Undefined property of $class instance: $k");
            return $nullval;
        }
    }
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof TapObject) {
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a TapObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if (($k === "metadata") && (is_array($v))) {
                $this->_values[$k] = TapObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToTapObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a TapObject. A TapObject may contain modified
            //      values within in that its parent TapObject doesn't know about.
            //
            $original = array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof TapObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if ($value === null) {
            ...
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToTapObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof TapObject) {
    ...
    /**
     * Returns a pretty JSON representation of the Tap object.
     *
     * @return string The JSON representation of the Tap object.
     */
    ...
    /**
     * Sets all keys within the TapObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any TapObjects contained as values or
    ...
    protected static function deepCopy($obj)
    {
        if (is_array($obj)) {
            ...
            return $copy;
        } elseif ($obj instanceof TapObject) {
    ...
    /**
     * @return object The last response from the Tap API
     */
    ...
    /**
     * Sets the last response from the Tap API
     *
